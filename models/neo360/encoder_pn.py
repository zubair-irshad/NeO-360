...
class SpatialEncoder(nn.Module):
    ...
    def index(self, uv, cam_z=None, image_size=(), z_bounds=None):
        ...
        with profiler.record_function("encoder_index"):
            ...
            #batched indexing
            # chunk = 50000
            # samples = torch.zeros((self.latent.shape[0],self.latent.shape[1],uv.shape[1], 1), device=self.latent.device, dtype=torch.float, requires_grad=self.latent.requires_grad)
            # for i in range(0, uv.shape[1], chunk):
            #     chunk_samples = F.grid_sample(
            #         self.latent,
            #         uv[:, i:i + chunk, :, :],
            #         align_corners=True,
            ...
            #     )
            #     detached_samples = chunk_samples.detach()
            #     samples[:,:,i:i + chunk,:] = detached_samples
